...
public class Schedule {
    ...
    public String toString() {
        ...
        for(int j=0; j<times.length; j++) {
            for(int i=0; i<times[j].length-1; i++) {
                strBuilder.append(times[j][i]);
                strBuilder.append(" ");
            }
            strBuilder.append(times[j][times[j].length-1]);
            strBuilder.append("\n");
        }
    ...
    public boolean equals(Object obj) {
        ...
        return pb.equals(otherSchedule.pb) && Instance.areMatrixEqual(times, otherSchedule.times);
    }
}
